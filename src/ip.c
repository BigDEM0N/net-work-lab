...
#define IP_MAX_DATA_LEN 1480
#define ETHERNET_MAX_TRANSPORT_UNIT 1500
#define IP_HDR_OFFSET_PER_BYTE 8
...
void ip_out(buf_t *buf, uint8_t *ip, net_protocol_t protocol)
{
    ...
    // 若无需分段，则直接发送
    if (buf->len <= ETHERNET_MAX_TRANSPORT_UNIT - IP_HDR_LEN)
    {
    ...
    // 分段发送，每段长度为1480B
    while (buf->len > IP_MAX_DATA_LEN)
    {
        buf_init(&fragment, IP_MAX_DATA_LEN);
        memcpy(fragment.data, buf->data, IP_MAX_DATA_LEN);
        buf_remove_header(buf, IP_MAX_DATA_LEN);
